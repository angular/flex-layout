...
if [ -z ${FLEX_LAYOUT_SCREENSHOT_FIREBASE_DEPLOY_TOKEN} ]; then
  echo "Error: No access token for firebase specified." \
       "Please set the environment variable 'FLEX_LAYOUT_SCREENSHOT_FIREBASE_DEPLOY_TOKEN'."
  exit 1
fi
...
$(npm bin)/firebase deploy --only functions --token ${FLEX_LAYOUT_SCREENSHOT_FIREBASE_DEPLOY_TOKEN} \
  --project flex-layout-screenshots
