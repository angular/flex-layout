...
# Publish Flex-Layout docs assets to the `flex-layout-docs-content` repo
# flex-layout.angular.io will pull from this assets repo to get the latest docs
...
if [ -z ${FLEX_LAYOUT_DOCS_CONTENT_TOKEN} ]; then
...
docsPath="./dist/docs"
packagePath="./dist/releases/flex-layout-examples"
repoPath="/tmp/flex-layout-docs-content"
repoUrl="https://github.com/angular/flex-layout-docs-content"
examplesSource="./dist/docs/examples"

if [[ ! ${*} == *--no-build* ]]; then
  $(npm bin)/gulp flex-layout-examples:build-release:clean
  $(npm bin)/gulp docs
fi
...
# Copy the flex-layout-examples package to the docs content repository.
...
echo "https://${FLEX_LAYOUT_DOCS_CONTENT_TOKEN}:@github.com" > .git/credentials
