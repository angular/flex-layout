...
do
  ...
  # The output for "lib" should go into a directory named "flex-layout"
  [[ ${p} = "lib" ]] && out_dir="flex-layout" || out_dir="${p}"
...
# Update the root @angular/flex-layout metadata file to re-export metadata from each entry-point.
./scripts/bazel/update-layout-metadata-reexports.js
