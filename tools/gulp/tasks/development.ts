...
import {buildConfig, copyFiles, watchFiles} from 'lib-build-tools';
...
task(':serve:devapp', series('aot:pre', execTask(
  'ng', ['serve', '--port', '5000'],
  {cwd: appDir, failOnStderr: true}
)));

task('build:devapp', series('aot:pre', execTask(
  'ng', ['build', '--prod'],
  {cwd: appDir, failOnStderr: true}
)));

task('serve:devapp', series(':serve:devapp', ':watch:devapp'));

/** Task that copies all vendors into the demo-app package. Allows hosting the app on firebase. */
task('stage-deploy:devapp', series('build:devapp',
 ...
 */
task('deploy:devapp', series('stage-deploy:devapp', () => {
