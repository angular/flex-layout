...
// `flex-layout-docs-example` attribute which can be used to locate the divs and initialize the example
// viewer.
const EXAMPLE_PATTERN = /<!--\W*example\(([^)]+)\)\W*-->/g;

// Markdown files can contain links to other markdown files.
// Most of those links don't work in the Flex-Layout docs, because the paths are invalid in the
...
task('highlight-examples', () => {
  ...
  return src('src/flex-layout-examples/**/*.+(html|css|ts)')
      .pipe(flatten())
...
task('plunker-example-assets', () => {
  src(path.join(packagesDir, 'flex-layout-examples', '**/*'))
...
function transformMarkdownFiles(buffer: Buffer, file: any): string {
  ...
  content = content.replace(EXAMPLE_PATTERN, (_match: string, name: string) =>
    `<div flex-layout-docs-example="${name}"></div>`
  );
...
/** Fixes paths in the markdown files to work in the flex-layout-docs-io. */
function fixMarkdownDocLinks(link: string, filePath: string): string {
  ...
  // guides can be loaded in the Flex-Layout docs.
