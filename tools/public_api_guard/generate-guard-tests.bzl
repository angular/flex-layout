...
def generate_test_targets(golden_files):
  for golden_file in golden_files:
    ...
    # entry point tail path. The package name is always the first path segment (e.g. "lib/")
    [package_name, entry_point_tail] = golden_file.split("/", 1)

    # Name of the entry-point (e.g. "core", "flex", "grid")
    entry_point = entry_point_tail[:-len(".d.ts")]

    # Construct the path to the given entry-point. Note that we also need to find a way to
    # allow guards for the primary entry-point of a package. e.g. "//src/lib:layout" should be also
    # validated. We achieve this by checking if the package_name is equal to the entry_point name.
    # For example: "public_api_guard/lib/core.d.ts" will be the golden for the primary entry-point.
    ...
    ts_api_guardian_test(
      ...
      actual = "angular_layout/src/%s/index.d.ts" % entry_point_path,
      data = [golden_file] + [
        "//src/%s" % (entry_point_path),
      ],
      golden = "angular_layout/tools/public_api_guard/%s" % golden_file,
