 ...
 */
export function MatchMediaObservableFactory(mediaWatcher: MatchMedia, breakpoints: BreakPointRegistry) {
  /**
  ...
  const observable$: Observable<MediaChange> = mediaWatcher.observe().filter(onlyActivations).map(injectAlias);
